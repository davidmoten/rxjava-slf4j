...
public class LoggingTest {
    ...
    public void testCountEvery() {
        int count = Observable
                .range(1, 10)
                // log all
                .lift(logger().name("rx.Server").prefix("count every test").excludeValue().subscribed(Level.DEBUG)
                        .onCompleted(Level.DEBUG).showCount("files").every(2).log())
                // count
                .count().toBlocking().single();
        assertEquals(10, count);
    }
    ...
    public void testSubscribe() {
        int count = Observable.range(11, 3010).lift(logger().showValue().showCount().every(1000).showMemory().log())
                .count().toBlocking().single();
