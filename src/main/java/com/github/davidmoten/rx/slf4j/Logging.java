...
public class Logging {
    ...
    public static class Parameters<T> {
        ...
        public static class Builder<T> {
            ...
            private Observable<Message<T>> observable = subject.materialize().map(
            ...
            public Builder<T> onNext(final boolean logOnNext) {
                this.observable = observable.filter(new Func1<Message<T>, Boolean>() {
            ...
            public Builder<T> showCount(final String label) {
                observable = observable.map(new Func1<Message<T>, Message<T>>() {
            ...
            public Builder<T> every(final int every) {
                if (every > 1)
                    observable = observable.filter(new Func1<Message<T>, Boolean>() {
                        AtomicLong count = new AtomicLong(0);
            ...
            public OperatorLogging<T> log() {
                return new OperatorLogging<T>(new Parameters<T>(getLogger(), subscribedMessage, unsubscribedMessage,
                        subscribedLevel, unsubscribedLevel, subject, observable.doOnNext(log)));
            }
            ...
            private final Action1<Message<T>> log = new Action1<Message<T>>() {
                ...
                public void call(Message<T> m) {
                    ...
                    if (m.value().isOnCompleted() && onCompleteMessage != null) {
                        ...
                        StringBuilder s = new StringBuilder();
                        addDelimited(s, String.format(onErrorFormat, m.value().getThrowable().getMessage()));
                        addDelimited(s, m.message());
                        addMemory(s);
                        Logging.log(getLogger(), s.toString(), onErrorLevel, m.value().getThrowable());
