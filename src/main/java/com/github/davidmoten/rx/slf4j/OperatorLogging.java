...
public class OperatorLogging<T> implements Operator<T, T> {
	...
	public Subscriber<? super T> call(Subscriber<? super T> child) {
		...
		Subscriber<Message<T>> logSubscriber = createErrorLoggingSubscriber(parameters
				.getLogger());
	...
	private static <T> Subscriber<Message<T>> createErrorLoggingSubscriber(
			final Logger logger) {
		return new Subscriber<Message<T>>() {
			...
			public void onError(Throwable e) {
				logger.error(
						"the logging transformations through an exception: "
								+ e.getMessage(), e);
			}
