...
public class OperatorLogging<T> implements Operator<T, T> {
    ...
    public Subscriber<? super T> call(Subscriber<? super T> child) {
        ...
        child.add(listener);
        Subscriber<T> parent = Subscribers.from(createObserver(p.getSubject(), child));
        child.add(parent);
    ...
    static <T> Observer<T> createObserver(final PublishSubject<T> subject, final Subscriber<? super T> child) {
